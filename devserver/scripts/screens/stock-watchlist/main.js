...
function buildStockRow(stock, theme) {
    ...
    return Row({
        ...
        Box({
            ...
            Icon({ name: "close", size: 12, color: theme.negative })
        ])
    ]);
}
...
function buildEmptyState(theme) {
    return Column({
        ...
        Text({ text: St("watchlist_empty"), fontSize: 14, color: theme.textSecondary })
    ]);
}
...
function render() {
    ...
    OnTheFly.setUI(Column({ height: "fill", background: theme.primary }, [
        ...
        Row({
            ...
            Text({ text: St("watchlist_title"), fontSize: 22, fontWeight: "800", color: theme.textPrimary, width: "wrap" }),
            Box({
                width: "wrap",
        ...
        Column({ id: "watchlist_scroll", fillMaxWidth: true, weight: 1, scrollable: true },
            scrollContent
        ),
